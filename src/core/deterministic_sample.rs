...
pub fn deterministic_sample<'a>(
    ...
    let unif_iter = deterministic_uniform_01_sample(n);
...
///
/// The sample covers the output range evenly throughout the generation process.
pub fn deterministic_uniform_01_sample(n: u64) -> impl Iterator<Item = f64> {
    UnifIter { n, i: 0 }
}
...
pub fn deterministic_uniform_sample(lo: f64, hi: f64, n: u64) -> impl Iterator<Item = f64> {
    deterministic_uniform_01_sample(n).map(move |v| (hi - lo) * v + lo)
}
